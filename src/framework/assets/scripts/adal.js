...
var AuthenticationContext = (function() {
  ...
  AuthenticationContext.prototype.acquireTokenRedirect = function(resource, extraQueryParameters, claims) {
    ...
    //TO BE REMOVED: Hack to bypass showing prompt if user logged in from iOS / Safari browser!
    var TRIDENT = /(msie|trident)/i.test(navigator.userAgent);
    var BLINK = typeof CSS !== 'undefined' && !this.EDGE && !this.TRIDENT;
    var EDGE = /(edge)/i.test(navigator.userAgent);
    var WEBKIT = /AppleWebKit/i.test(navigator.userAgent) && !EDGE && !TRIDENT;
    var IOS = /iPad|iPhone|iPod/.test(navigator.userAgent) && !('MSStream' in window);
    var SAFARI = /safari/i.test(navigator.userAgent) && WEBKIT;
    var MOBILE_CHROME_IOS = /CriOS/i.test(navigator.userAgent) && IOS;
    // urlNavigate = urlNavigate + '&prompt=select_account';

    try {
      if (window.appInsights) {
        window.appInsights.trackException(new Error(), null, {
          upn: this._user.profile.upn,
          iOS: IOS,
          safari: SAFARI
        });
      }
    ...
    try {
      if (!IOS && !SAFARI) {
        urlNavigate = urlNavigate + '&prompt=select_account';
      } else if (IOS && SAFARI && !MOBILE_CHROME_IOS) {
        urlNavigate = urlNavigate + '&prompt=none';
      } else {
        urlNavigate = urlNavigate + '&prompt=select_account';
      }
  ...
  AuthenticationContext.prototype.saveTokenFromHash = function(requestInfo) {
    ...
    if (requestInfo.parameters.hasOwnProperty(this.CONSTANTS.ERROR_DESCRIPTION)) {
      this.infoPii(
        'Error :' + requestInfo.parameters.error + '; Error description:' + requestInfo.parameters[this.CONSTANTS.ERROR_DESCRIPTION]
      );
    ...
    if (level <= Logging.level) {
      ...
      if (this.config.correlationId)
        formattedMessage =
          ...
          ' ' +
          message;
      else formattedMessage = timestamp + ':' + this._libVersion() + '-' + this.CONSTANTS.LEVEL_STRING_MAP[level] + ' ' + message;
